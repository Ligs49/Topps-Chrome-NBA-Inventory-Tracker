...
PRODUCT_CONTEXT_TERMS = [
    "basketball", "nba", "trading card", "trading cards",
    "sports card", "sports cards", "value box", "mega box", "blaster box",
]

UPDATE_TERMS = [
    "update", "updates", "update series", "chrome update", "chrome updates",
]

def product_match_strength(text):
    lower = normalize_text(text)
    if "topps" not in lower or "chrome" not in lower:
        return 0
    if not any(term in lower for term in PRODUCT_CONTEXT_TERMS):
        return 0
    return 2 if any(term in lower for term in UPDATE_TERMS) else 1

def relevant_product_window(text):
    lower = normalize_text(text)
    positions = []
    for term in ("topps", "chrome"):
        start = 0
        while True:
    ...
    if not positions:
        return lower
    pieces = []
    for idx in positions[:12]:
        lo = max(0, idx - 450)
        hi = min(len(lower), idx + 900)
        pieces.append(lower[lo:hi])
    return " ".join(pieces)

def local_context_score(text, store):
    lower = normalize_text(text)
    score = 0
    city = store["city"].lower()
    zipcode = store["zip"]
    number_match = re.match(r"(\d+)", store["address"])
    ...
    if zipcode in lower:
        score += 3
    if number and number in lower:
        score += 2

    street_part = store["address"].lower().split(",")[0]
    stop = {"st","street","rd","road","ave","avenue","dr","drive","blvd","boulevard","ste","suite"}
    street_words = [w for w in re.findall(r"[a-z0-9]+", street_part) if len(w) >= 4 and w not in stop]
    score += min(sum(1 for w in street_words if w in lower), 2)
    return score

def local_context_present(text, store):
    return local_context_score(text, store) >= 3

def classify_local_text(text, store, max_price):
    full = normalize_text(text)
    product_text = relevant_product_window(full)
    strength = product_match_strength(product_text)

    if strength == 0:
        return result(
            "NO LISTING",
            "No matching Topps Chrome basketball/card listing found.",
            None,
        )

    # Only parse price after a product match; avoids unrelated $21.99-type prices.
    price = choose_price(product_text, max_price)
    local = local_context_present(full, store)
    score = local_context_score(full, store)

    local_out_phrases = [
        "pickup not available", "out of stock", "not available at this store",
        "unavailable for pickup", "sold out", "not available nearby",
        "unavailable nearby",
    ]
    local_in_phrases = [
        "pickup available", "ready for pickup", "pickup today",
        "available for pickup", "get it today", "in stock", "available today",
    ]
    generic_online_phrases = [
        "add to cart", "add to basket", "shipping", "ship it",
        "delivery", "available online",
    ]

    phrase_text = product_text + " " + full

    if local and any(p in phrase_text for p in local_out_phrases):
        return result(
            "OUT OF STOCK",
            f"Local store context verified (score {score}) and availability is negative.",
            price,
        )

    if local and any(p in phrase_text for p in local_in_phrases):
        return result(
            "IN STOCK",
            f"Local store context verified (score {score}) with an in-stock/pickup signal.",
            price,
        )

    if any(p in product_text for p in generic_online_phrases):
        return result(
            "ONLINE LISTING ONLY",
            "Matching product listing found online, but exact local-store pickup was not verified.",
            price,
        )
    ...
    if strength == 2:
        return result(
            "CHECK MANUALLY",
            "Topps Chrome Updates product matched, but no reliable local inventory signal was exposed.",
            price,
        )

    return result(
        "CHECK MANUALLY",
        "Probable Topps Chrome basketball/card listing found, but local inventory could not be verified.",
        price,
    )
    ...
    try:
        with sync_playwright() as p:
            ...
            if store["store"] == "Target":
                for label in ["Edit store", "Change store", "Check other stores", "Select your store", "My store"]:
                    try:
            ...
            # Walmart patterns
                ...
                for phrase in ["Pickup", "How do you want your items", "Set location", "Change", "Select store"]:
                    try:
